...
Map a `(parameter_step, iteration)` pair from a sweep configuration to a
single linear 1-based index.

The mapping assumes sweep results are stored in blocks by `parameter_step`, with
all iterations for one step placed contiguously.

# Keyword Arguments
- `iteration::Int`: Iteration number within a parameter step (1-based).
- `parameter_step::Int`: Parameter step in the sweep (1-based).
- `parameter`: Object that provides `parameter_steps` and `iterations`; the
    first element of each is used as the corresponding upper bound.
...
```
"""
function index(; iteration::Int = 1, parameter_step::Int = 1, parameter = data_used)
    max_steps = parameter.parameter_steps[1]
    max_iter  = parameter.iterations[1]

    if parameter_step < 1 || parameter_step > max_steps || iteration < 1 || iteration > max_iter
        throw(ArgumentError("iteration or parameter_step out of bounds"))
    end

    return (parameter_step - 1) * max_iter + iteration
end